...
yy.create=function(prop){//prop.arr->模式;prop.cipai->词牌名(NOT 代号)
	...
	yy.show.innerHTML="<span style='position:absolute;top:0;right:0;cursor:pointer;font-size:20px;' title='Hide' "+
						...
						"<div style='margin:10% 5px;'><div style='line-height:25px;font-weight:bold;'>"+prop.cipai+"</div><div style='line-height:20px;'>"+str+"</div></div>";
...
yy.descBalls=function(num){//初始化balls
	var circle;
	var num=(num!=undefined)?parseInt(num):15;
	for(var i=0;i<num;i++){
...
yy.update=function(){//更新每个ball的属性
	for(var i=0;i<yy.circles.length;i++){
		var x=yy.circles[i].x+yy.circles[i].vx;
		var y=yy.circles[i].y+yy.circles[i].vy;
...
window.yy_requestNextAnimationFrame=
(function(){
